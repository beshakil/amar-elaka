...
Bangladesh administrative boundaries from **HDX COD-AB (sourced from BBS), ADM0–ADM4**: country → division → district → upazila / urban thana → union / ward. Source, licence, attribution and import procedure: [ADR 003](../decisions/003-geo-data-source.md).
**Scope:** GLOBAL
...
| `kind_code`                    | `text → category_kinds`   | NO   | —       | marketplace / service / job / rental / place (business directory). Decides which table uses it.                                                                                          |
...
**Keys:** PK `id`. `parent_id → categories` RESTRICT. CHECK: child's `kind_code` equals parent's (trigger).
**Indexes:**
...
| `ui_schema`            | `jsonb`                  | NO   | `'{}'`    | Field order, widget hints, i18n label keys. No literal labels.                                                                                                                                                                                                                                         |
...
| `status_code`          | `text → schema_statuses` | NO   | `'draft'` | draft / published / retired.                                                                                                                                                                                                                                                                           |
...
- `unique (category_id) where status_code = 'published'`: exactly one live schema per category.
  **RLS:** G-REFERENCE. A published row is immutable (trigger), because posts pin to it.
...
| `storage_key`         | `text`                      | NO   | —                  | Object key, e.g. `t/<tenant>/2026/09/<id>.webp`.                                                                                                                                                                                                                       |
| `mime_type`           | `text`                      | NO   | —                  | Sniffed server-side, not trusted from client.                                                                                                                                                                                                                          |
| `byte_size`           | `bigint`                    | NO   | —                  | CHECK > 0.                                                                                                                                                                                                                                                             |
| `width`               | `integer`                   | YES  | —                  |                                                                                                                                                                                                                                                                        |
| `height`              | `integer`                   | YES  | —                  |                                                                                                                                                                                                                                                                        |
| `duration_ms`         | `integer`                   | YES  | —                  | Video only.                                                                                                                                                                                                                                                            |
| `checksum_sha256`     | `text`                      | NO   | —                  | Dedupe and known-bad-image matching.                                                                                                                                                                                                                                   |
| `blurhash`            | `text`                      | YES  | —                  | Placeholder on slow 3G.                                                                                                                                                                                                                                                |
| `variants`            | `jsonb`                     | NO   | `'{}'`             | `{"thumb":"key","md":"key"}` generated by worker.                                                                                                                                                                                                                      |
...
| `category_id`               | `uuid`                         | NO   | —                   | Must be enabled in tenant (service check). Kind ≠ `place` (trigger).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
...
| Catalog            | `geo_area_levels`, `category_kinds`, `schema_statuses`                                                                                                                                                                                                                                                                                                                                                                                                                |
...
| `discover_nearby(point, radius_km, kinds, filters)`                         | DB fallback for pure-radius discovery across tenants. Returns `(entity, id, tenant_id, distance_m)` for **public-state rows only**; details are then read in each owner's context. |
