/**
 * ThumbHash encoder: a ~25-byte placeholder that decodes to a blurry
 * preview with the right colours, aspect ratio and alpha, shown while the
 * real image loads on a slow connection.
 *
 * Port of `rgbaToThumbHash` from https://github.com/evanw/thumbhash
 * (Evan Wallace, MIT licence). The numeric constants are the algorithm's own
 * (DCT sizes, quantisation scales), not tunable business values.
 */

/** ThumbHash only accepts images up to 100x100. */
export const THUMBHASH_MAX_SIZE = 100;

export function rgbaToThumbHash(w: number, h: number, rgba: Uint8Array): Uint8Array {
  if (w > THUMBHASH_MAX_SIZE || h > THUMBHASH_MAX_SIZE) {
    throw new Error(`${w}x${h} doesn't fit in ${THUMBHASH_MAX_SIZE}x${THUMBHASH_MAX_SIZE}`);
  }
  ...
  const [a_dc, a_ac, a_scale] = hasAlpha ? encodeChannel(a, 5, 5) : ([0, [], 0] as const);
